...
#[test]
fn torrentzip_rvzstd_crc_check() {
    let dir = tempdir().unwrap();
    let src = dir.path().join("rom_zstd.bin");
    let mut f = File::create(&src).unwrap();
    f.write_all(b"hello_rvzstd").unwrap();
    ...
    let rec = FileRecord {
        source: src.clone(),
        relative: std::path::PathBuf::from("rom_zstd.bin"),
        size: 12,
    ...
    assert!(comment.starts_with("RVZSTD-") || comment.starts_with("TORRENTZIPPED-"));
    let parsed = if comment.starts_with("RVZSTD-") {
